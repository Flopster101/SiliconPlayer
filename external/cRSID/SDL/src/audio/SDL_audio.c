...
int SDLCALL SDL_RunAudio(void *audiop)
{
	...
	while ( audio->enabled ) {
		...
		SDL_memset(stream, silence, stream_len);  //might eat considerable extra CPU-time
		...
		if ( stream == audio->fake_stream ) {
			...
			audio->WaitAudio(audio); //SDL_Delay((audio->spec.samples*(1000/4))/audio->spec.freq); //audio->WaitAudio(audio);//, audio->spec.samples, audio->spec.freq );  //when compiled SDL internally this didn't wait and caused a tight loop eating CPU
		}                                 //it seems blocking-mode didn't work and originally WaitAudio() was empty, now it delays a bit
	}
