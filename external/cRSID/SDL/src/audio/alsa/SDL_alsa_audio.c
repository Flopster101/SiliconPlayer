...
static void ALSA_WaitAudio (_THIS); //, int bufferlength, int samplerate);
static int ALSA_getAvailableSamplesToWrite (_THIS); //added by Hermit
...
static void ALSA_WaitAudio (_THIS) //, int bufferlength /*samples*/, int samplerate /*Hz*/ )
{  enum { MILLISECONDS_IN_SECOND = 1000, BUFFERLENGTH_MIN = 128, DEFAULT_SAMPLERATE = 44100, WAIT_MIN = BUFFERLENGTH_MIN * MILLISECONDS_IN_SECOND/DEFAULT_SAMPLERATE };
 while ( ALSA_getAvailableSamplesToWrite(this) <= this->spec.samples /*BUFFERLENGTH_MIN*/ ) SDL_Delay( WAIT_MIN ); //SDL_Delay( bufferlength * MILLISECONDS_IN_SECOND/samplerate ); ////2 /* We're in blocking mode, so there's nothing to do here */
}  //the assumption about blocking-mode haven't always worked out (SIDwizard_intSDL), so added a little bit of delay to at least avoid 100% CPU-hogging
...
static int ALSA_getAvailableSamplesToWrite (_THIS) { //added by Hermit  //printf("%d\n", (int) snd_pcm_avail_update( pcm_handle ) );
 if (pcm_handle != NULL) return (int) snd_pcm_avail_update( pcm_handle );
 else return -1;
}
